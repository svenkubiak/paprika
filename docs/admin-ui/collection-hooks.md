...
- **URL** and **method** — where the request is sent (default `POST`). Must be `http` or `https`. The URL's host is resolved and checked at save time: it can't point at loopback, link-local, private/site-local, or multicast addresses — so a hook can't target `localhost`, `127.0.0.1`, or an internal-network IP. Point hooks at a publicly reachable endpoint (or a tunnel like ngrok during local development).
