...
- **Forward request headers** — additional incoming request headers to include in `context.http.headers`, as a comma-separated list (e.g. `x-app-key-id, x-signature`). Empty by default, which means only the fixed allowlist below is forwarded. Header names are stored lowercased and matched case-insensitively, so `X-App-Key-Id` also matches a client sending `x-app-key-id`. Every name you add here is **sent to the configured hook URL**, which may be a third party — list only what the target actually needs to decide. `Authorization`, `Cookie`, `Set-Cookie` and `Proxy-Authorization` can never be forwarded: they are credentials against Paprika itself, so a hook that receives one could impersonate the caller. Saving a hook that lists one of them fails with `400` rather than silently dropping it.
...
- **`context.http.headers`** contains a fixed, non-sensitive subset of the incoming request headers: `Content-Type`, `User-Agent`, `Accept`, `Accept-Language` and `X-Request-Id` — plus any header the hook explicitly opts into through **Forward request headers** (`forwardHeaders`). Nothing else is forwarded, since a hook URL may point at a third party: a header the client sent but no hook asked for (e.g. `X-Api-Key`) stays out, and `Authorization`, `Cookie`, `Set-Cookie` and `Proxy-Authorization` are never forwardable at all, not even when configured. A `beforeRequest` hook that acts as an external authorizer is the case `forwardHeaders` exists for: it can be given the device proof, signature, or idempotency key a client sends to legitimize itself. If your endpoint needs a secret of its own, configure it as a static outgoing header on the hook instead.
