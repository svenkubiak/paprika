...
- **URL** and **method** — where the request is sent (default `POST`). Must be `http` or `https`. The URL's host is resolved and checked, both at save time and again on every dispatch: it can't point at loopback, link-local, private/site-local, or multicast addresses — so a hook can't target `localhost`, `127.0.0.1`, or an internal-network IP — unless the tenant's **webhook allowlist** (configured on the [Auth settings](/admin-ui/auth-settings#webhook-allowlist) page) explicitly lists that exact `host:port`. This is a per-tenant escape hatch, not an instance-wide switch: on a shared multi-tenant instance, one tenant allowlisting a host never affects what any other tenant can reach. Point hooks at a publicly reachable endpoint (or a tunnel like ngrok during local development) unless you've deliberately allowlisted a local target.
